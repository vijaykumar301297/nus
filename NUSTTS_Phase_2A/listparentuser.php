...
include ('dbconn.php');
    $sql = "select * from nususerdata where role=\"Parent Company\" AND active=\"Active\"";
    ...
    <thead>
		...
		<tbody>
			<?php
                ...
                ?>
                <tr>       
                    <td><input type="checkbox" class="listparentuser" name="selectData[]" value=<?=$rows['id']?>></td>
     ...
     <button type="submit" name="disableuser" class=" btn-warning list_parent disable" ><img class= "disableIcon" src="img/Disable_icon.svg" alt="">Disable User</button>
 </form>
</body>
<?php
...
?>
    ...
    $('.listparentuser').each(function(){
        $(this).on('click',function(){
        var lengths = $('.listparentuser:checkbox:checked').length;      
        if(lengths >=1 ){
            $('.list_parent').show();
        }else{
            $('.list_parent').hide();
