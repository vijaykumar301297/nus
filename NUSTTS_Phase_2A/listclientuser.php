...
include ('dbconn.php');
    $sql = "select * from nususerdata where role=\"Client Company\" AND active=\"Active\"";
...
<form action="" method="POST">
    ...
    <thead>
		...
		<tbody>
			<?php
                ...
                ?>
                <tr>       
                    <td><input type="checkbox" class="listclientuser" name="selectData[]" value=<?=$rows['id']?>></td>
                ...
                <?php
                    ...
                    <td><?php
                            $newstr = trim($rows['bussinessunit']);
                            $newVal = explode(" ",$newstr);
                        	$output = implode(",",$newVal);
                        	$sqlQueryOnee = "SELECT * FROM clientcompanydata WHERE id IN ($output);";
                            $resultOneQuery = mysqli_query($conn,$sqlQueryOnee);
                            $finalRes = '';
                            while($rowOneQuery = mysqli_fetch_assoc($resultOneQuery)) {
                                $finalRes = $rowOneQuery['clientcompany'].", ".$finalRes;
                            }
                            $resVal = trim($finalRes);
                            echo trim($resVal,",");
                        ?>
				...
				<div class="btn-group">
                    ...
                    <ul class="dropdown-menu" role="menu">
                            ...
                            <li>
                                <a href="moveParentClient.php?id=<?=$rows['id']?>" id="moving">Move User</a>
                            </li>
                            <li>
    ...
    <button type="submit" name="disableuser" class=" btn-warning list_client disable" ><img class= "disableIcon" src="img/Disable_icon.svg" alt="">Disable User</button>
 </form>
</body>
<?php
...
<script>
    ...
    $('.listclientuser').each(function(){
        $(this).on('click',function(){
        var lengths = $('.listclientuser:checkbox:checked').length;      
        if(lengths >=1 ){
            $('.list_client').show();
        }else{
            $('.list_client').hide();
