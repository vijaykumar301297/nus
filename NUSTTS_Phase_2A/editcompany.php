...
<body>
    <?php
    include "dbconn.php";
    $parentcompany = mysqli_query($conn, "SELECT * From parentcompanydata WHERE id=".$_GET['id']."");  
    $parentedit = array();
    while($parentcompanydata = mysqli_fetch_array($parentcompany))
    {
        $parentedit[] = $parentcompanydata;
    }
    // print_r($parentedit);
    ?>
    <section class="sec1Container">
        <div class="sec1Wrapper">
            <div class="parentContainer">
                ...
                <hr color="#d2ddec">
                <form action="editparentcompany.php" method="POST" class="parentAddForm">
                    <label for="pCompany">Parent entity name</label>
                    <input type="text" name="parentcompany" id="pCompany" placeholder="Enter the name of the parent entity" required value="<?=$parentedit[0]['parentcompany']?>">
                    <input type="hidden" name="parentid" value="<?=$parentedit[0]['id']?>">
                    <hr color="#d2ddec">
                    <div class="one">
                        ...
                        <button type="submit" class="createClass">Update Parent <img src="img/plus-white.svg" alt="plus icon" width="14px"></button>
                    </div>
                </form>
            </div>
        </div>
