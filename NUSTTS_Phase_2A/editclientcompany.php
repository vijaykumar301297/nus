<?php
    ...
    if($_SESSION['role'] != 'Admin' && $_SESSION['role'] != 'NUS User' && $_SESSION['role'] != 'NUS Manager'){
...
<head>
    ...
    <title>NUS TTS System | Create Company</title>
    <link rel="icon" href="img/social-square-n-blue.png">
    <link rel="stylesheet" href="css/client.css">
</head>
<?php
include "dbconn.php";  // Using database connection file here
$clientCompany = mysqli_query($conn, "SELECT * From clientcompanydata WHERE id=".$_GET['id']."");  
$clientArray = array();
while($row = mysqli_fetch_array($clientCompany))
{
    $clientArray[] = $row;
}   
print_r($clientArray);
    ...
    <section class="sec1Container">
        <div class="sec1Wrapper">
            <div class="clientContainer">
                ...
                <hr color="#d2ddec">
                <form action="postclientcompanydata.php" method="POST" class="clientAddForm">
                    <label for="cCompany">Client name</label>
                    <input type="text" name="clientcompany" id="cCompany" value="<?=$clientArray[0]['clientcompany']?>" placeholder="Enter the client company's name" required>
                    <label for="pCompany">Parent Company</label>
                    <select name="parentcompany" id="pCompany" required>
                    <option disabled>Select a parent company</option>
                    <?php
                        ...
                        {
                            $selected ='';
                            if($parentcompanydata['parentcompany'] == $clientArray[0]['parentcompany']){
                                $selected = 'selected';
                            }
                            echo "<option value='". $parentcompanydata['parentcompany'] ."' ".$selected.">" .$parentcompanydata['parentcompany']."</option>"; 
                        }
                    ?>  
                    </select>
                    <label for="country">Country</label>
                    <select name="country" id="country" class="clientSelLast" required>
                ...
                <option value="Zimbabwe">Zimbabwe</option>
                    ...
                    <div class="one">
                        ...
                        <button type="submit" class="createClass">Create Client <img src="img/plus-white.svg" alt="plus icon" width="14px"></button>
                    </div>
                </form>
            </div>
        </div>
